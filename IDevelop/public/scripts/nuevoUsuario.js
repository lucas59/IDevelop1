	...
	function nuevoUsuario(){
		...
		if(existe == "1"){
			...
			link.setAttribute("href", urlBase+"/Usuario/login");
			$("#modalAviso").modal();
	...
	function enviarValidacion(email,nombre,apellido,token){
		...
		$.ajax({
			async:false,
			url: urlBase+'/Usuario/Validacion/Enviar',
			type: 'POST',
			data: {
				"email": email,
				"nombre": nombre,
	...
	function ingresarUsuario(email,nombre,apellido,contraseña,fecha,sexo,tipo,token){
		...
		$.ajax({
			async:false,
			url: urlBase+'/Usuario/NuevoUsuario',
			type: 'POST',
			data: {
				"email": email,
				"nombre": nombre,
	...
	function validarEmail(email){
		...
		$.ajax({
			async:false,
			url: urlBase+'/Usuario/validarCorreo/'+email,
		//data:{"email":email},
		success: function(response){
			response = response.trim();
